...
function SelectComponent({ title = "Select", data, handleChange = () => {} }) {
    ...
    const isSelectedItem = (value) => {
        return listSelected.findIndex((item) => item.value === value) !== -1;
    };
    ...
    return (
        ...
        >
            <div>
                <label
                    className="text-xs z-20 absolute -top-2 left-2 px-1 font-bold bg-white"
                    htmlFor={id}
                >
                    {title}
                </label>
                <div className="w-full relative flex items-center">
                    <div className="flex p-[9px] items-center flex-1 flex-wrap">
                        {listSelected &&
                            listSelected.length > 0 &&
                            listSelected.map((item, index) => (
                                <div
                                    key={item.value + index}
                                    className="cursor-pointer hover:opacity-100 opacity-90 transition-all delay-50 ease-linear flex text-[13px] font-bold bg-[#d6f4f9] ml-2 rounded-lg p-1 text-[#006C9C]"
                                >
                        ...
                        <input
                            ...
                            className={
                                "outline-none bg-white py-[7.5px] ml-2 rounded-lg text-sm flex-1 relative"
                            }
                            type="text"
                        />
                    </div>
